; procedures for change of netCDF attributes in WRF products
; 

function w_change_WRF_product_attributes_file, file, TITLE=title
  ...
  case (DOMAIN) of
    ...
    2: dStr = 'd10km'
    3: dStr = 'd02km'
    else: Message, 'Global "DOMAIN" attribute not in correct format.'
  endcase
  ...
  sObj->WriteGlobalAttr, 'TITLE', ITITLE
...
;
; :Params:
;   directory (in, required): The directory to search for files
;   title     (in, required): The title to be written in the netCDF files. 
;                             E.g. for HAR: "HAR v2"
;                                  for CER: "Central Europe Refined analysis - CER V1"
;   pattern   (in, optional): The Pattern to use for searching files, default is '*.nc'
;   savefile  (in, optional): Optional, information which files are ok is saved in the savefile
;   info     (out, optional): A struct with the tags ok and file for each processed file.

;-

pro w_change_WRF_product_attributes, directory, title, PATTERN=pattern, SAVEFILE=savefile, INFO=info
  ...
  if N_PARAMS() NE 2 then begin
    message, "No directory and/or title given."
  endif
  ...
  for i=0, cnt-1 do begin
    inF = files[i]
    ok[i] = w_change_WRF_product_attributes_file(inF, TITLE=title)
    if TNT_is_gui then begin
