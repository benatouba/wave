;+
; :Description:
;    Computes the root mean square mean deviation between two datasets.
;
...
;     Written by FaM, 2012.
;-
function w_BCRMSD, ref, data, VALID=valid
  ...
  case size(ref, /N_DIMENSIONS) of
    1: begin
      diff = _data - w_MD(_ref, _data) - _ref
      return, SQRT(mean(diff*diff, /NAN))
    end
    2: begin
      md = (w_MD(_ref, _data)) # (FLTARR((size(ref, /DIMENSIONS))[1]) + 1)
      diff = _data - md - _ref
      return,  SQRT(mean(diff*diff, /NAN, DIMENSION=2))
    end
